...
\begin{solution}
    ...
    \begin{align*}
        \text{Assume: } & y_{1p}(x) = A \\
                        ...
                        & 10A = 20 \\
                        & A = 2 \\
                        \Longrightarrow & y_{1p}(x) = 2    \end{align*}

    Solving for $y_{1g}(x)$ and $y_{1g}'(x)$.
    \begin{align*}
        & y_{1g}(x) = y_{1c}(x) + y_{1p}(x) \\
        & y_{1g}(x) = e^{x} \left(C_{1}\cos\left(3 x\right) + C_{2}\sin\left(3 x\right)\right) + 2 \\
        & y_{1g}'(x) = e^{x} \left(C_{1}\cos\left(3 x\right) + C_{2}\sin\left(3 x\right)\right) + e^{x} \left(-3C_{1}\sin\left(3 x\right) + 3C_{2}\cos\left(3 x\right)\right) \\
        & y_{1g}'(x) = e^{x} \left(\left(C_{1} + 3C_{2}\right)\cos\left(3 x\right) + \left(C_{2} - 3C_{1}\right)\sin\left(3 x\right)  \right)
    \end{align*}

    Plugging in for constants.
    \begin{align*}
        & y(0) = 0, \ y'(0) = 0 \\
        & y_{1g}(0) = \cancel{e^{\left(0\right)}} \left(C_{1}\cos\left(3 \left(0\right)\right) + \cancel{C_{2}\sin\left(3 \left(0\right)\right)}\right) + 2 = 0 \\
        & y_{1g}(0) = C_{1} + 2 = 0 \\
        & y_{1g}(0) = C_{1} = -2 \\
        & y_{1g}'(0) = \cancel{e^{\left(0\right)}} \left(\left(C_{1} + 3C_{2}\right)\cos\left(3 \left(0\right)\right) + \cancel{\left(C_{2} - 3C_{1}\right)\sin\left(3 \left(0\right)\right)}  \right) = 0 \\
        & y_{1g}'(0) = C_{1} + 3C_{2} = 0 \\
        & y_{1g}'(0) = (-2) + 3C_{2} = 0 \\
        & y_{1g}'(0) = C_{2} = \frac{2}{3}
    \end{align*}

    Combine for $y_{1}(x)$ for range of $0 \leq x \leq \pi$. 
    \begin{align*}
        & C_{1} = -2, \quad C_{2} = \frac{2}{3} \\
        & y_{1g}(x) = e^{x} \left(C_{1}\cos\left(3 x\right) + C_{2}\sin\left(3 x\right)\right) + 2 \\
    \Longrightarrow & \boxed{y_{1}(x) = e^{x} \left(-2\cos\left(3 x\right) + \frac{2}{3}\sin\left(3 x\right)\right) + 2}
    \end{align*}

    Now the same thing for $y_{2}(x)$ for the range of $x > \pi$. We should recognize this is just the homogenous, complementary part from $y_{1}(x)$ so 
    we can skip and state the following. There is no particular solution as it is homogenous.
    \begin{align*}
        & y'' -2y' + 10y= 0 \\
    \Longrightarrow    & y_{2g}(x) = y_{1c}(x) \\
        & \boxed{y_{2g}(x) =  e^{x} \left(C_{3}\cos\left(3 x\right) + C_{4}\sin\left(3 x\right)\right)}
    \end{align*}    

    Now have to have to satisfy continuity and smoothness continuity for our constants.
    \begin{align*}
        & y_{1}(\pi) = y_{2}(\pi) \\
        & y_{1}(x) = e^{x} \left(-2\cos\left(3 x\right) + \frac{2}{3}\sin\left(3 x\right)\right) + 2 \\
        & y_{1}(x) = e^{\left(\pi\right)}\left(-2\cos\left(3 \left(\pi\right)\right) + \cancel{\frac{2}{3}\sin\left(3 \left(\pi\right)\right)}\right) + 2\\
        & y_{1}(\pi) = 2e^{\pi} + 2\\
        & y_{2}(x) = e^{x} \left(C_{3}\cos\left(3 x\right) + C_{4}\sin\left(3 x\right)\right) \\
        & y_{2}(\pi) = e^{\left(\pi\right)}\left(C_{3}\cos\left(3 \left(\pi\right)\right) + \cancel{c_{4}\sin\left(3 \left(\pi\right)\right)}\right) \\
        & y_{2}(\pi) = -C_{3}e^{\pi} \\
        & y_{1}(\pi) = y_{2}(\pi) \\
        & 2e^{\pi} + 2 = -C_{3}e^{\pi} \\
        & C_{3} = -2-2e^{-\pi}\\\\
        & y_{1}'(\pi) = y_{2}'(\pi) \\
        & y_{1}'(x) = e^{x} \left(\left(C_{1} + 3C_{2}\right)\cos\left(3 x\right) + \left(C_{2} - 3C_{1}\right)\sin\left(3 x\right)  \right) \\
        & y_{1}'(x) = e^{\left(\pi\right)} \left(\left(C_{1} + 3C_{2}\right)\cos\left(3 \left(\pi\right)\right) + \cancel{\left(C_{2} - 3C_{1}\right)\sin\left(3 \left(\pi\right)\right)}   \right)\\
        & y_{1}'(\pi) = -e^{\pi}\left(C_{1} + 3C_{2}\right)  \\
        & y_{1}'(\pi) = -e^{\pi}\left(-2 + 3\frac{2}{3}\right) \\
        & y_{1}'(\pi) = 0 \\
        & y_{2}'(x) = e^{x} \left(\left(C_{3} + 3C_{4}\right)\cos\left(3 x\right) + \left(C_{3} - 3C_{4}\right)\sin\left(3 x\right)  \right) \\
        & y_{2}'(\pi) = -e^{\pi}\left(C_{3} + 3C_{4}\right) \\
        & y_{1}'(\pi) = y_{2}'(\pi) \\
        & 0 = \cancel{-e^{\pi}}\left(C_{3} + 3C_{4}\right) \\
        & 0 = -2-2e^{-\pi} + 3C_{4}\\
        & C_{4} = \frac{2+2e^{-\pi}}{3} \\\\
        & y_{2g}(x) = e^{x} \left(C_{3}\cos\left(3 x\right) + C_{4}\sin\left(3 x\right)\right) \\
        & y_{2}(x) = e^{x} \left(\left(-2-2e^{-\pi}\right)\cos\left(3 x\right) +\left(\frac{2+2e^{-\pi}}{3}\right) \sin\left(3 x\right)\right) \\
        \Longrightarrow & \boxed{y_{2}(x) = e^{x}\left(1+e^{-\pi}\right) \left(-2\cos\left(3 x\right) + \frac{2}{3}\sin\left(3 x\right)\right)}
    \end{align*}
    ...
    \begin{equation*}
        ...
        \boxed{y(x) = \begin{cases}
                  e^{x} \left(-2\cos\left(3 x\right) + \frac{2}{3}\sin\left(3 x\right)\right) + 2    & 0 \leq x \leq \pi  \\[2mm]
                  e^{x}\left(1+e^{-\pi}\right) \left(-2\cos\left(3 x\right) + \frac{2}{3}\sin\left(3 x\right)\right)     & x > \pi
              \end{cases}}
    \end{equation*}
...
Cramer's rule to find our $u_{1}'$ and $u_{2}'$ to find our particular solution $y_p(\theta)$.

Solving for $y_{c}(\theta)$.
\begin{align*}
    & y''+y=\sec \theta \tan \theta \\
    & m^2 + 1 = 0 \\
    & m = \pm i \\
    \Longrightarrow & \boxed{y_{c}(\theta) = C_{1} \cos \left(\theta\right) + C_{2} \sin \left(\theta\right)}
\end{align*}
