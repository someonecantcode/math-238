...
\begin{enumerate}
    ...
    \begin{solution}
        \begin{align*}
            x + 3y^2 \sqrt{x^{2}+1} \frac{dy}{dx} &= 0 \\
            3y^2 dy &= \frac{-x}{\sqrt{x+1}} dx \\
            \int 3y^2 dy &= \int \frac{-x}{\sqrt{x^2+1}} dx \\\\
            \text{Let } u &= x^2+1 \\
            \frac{du}{2}&= x \ dx\\
            y^3 &= \int -\frac{1}{2} u^{-1/2} du \\
            y^3 &= -u^{1/2} + C \\
            \Longrightarrow  y^3 &= - \sqrt{x^2+1} + C \\\\
            y(0) &= 1 \\
            (1)^3 &= - \sqrt{(0)^2+1} + C \\
            C &= 2, \ y^3 = - \sqrt{x^2+1} + 2 \\
            \Longrightarrow \quad & \boxed{y(x) = \sqrt[3]{2 - \sqrt{x^2+1}}} 
        \end{align*}
    \end{solution}
